...
class unet3d(nn.Module):
    def __init__(self, init_channels=1, class_nums=1, batch_norm=True, sample=True):
        ...
        self.down1 = pub(init_channels, 8, batch_norm)
        self.down2 = unet3dDown(8, 16, batch_norm)
        self.down3 = unet3dDown(16, 32, batch_norm)
        self.down4 = unet3dDown(32, 64, batch_norm)
        self.down5 = unet3dDown(64, 128, batch_norm)
        self.up4 = unet3dUp(128, 64, batch_norm, sample)
        self.up3 = unet3dUp(64, 32, batch_norm, sample)
        self.up2 = unet3dUp(32, 16, batch_norm, sample)
        self.up1 = unet3dUp(16, 16, batch_norm, sample)
        self.con_last = nn.Conv3d(16, class_nums, 1)
...
if __name__ == '__main__':
    ...
    inputs = torch.randn(1, 1, 80, 128, 144).to(device) # 这里的对应前面fforward的输入是32
    net = unet3d(1,14,batch_norm=True, sample=True).to(device)
    flops, params = profile(net, inputs=(inputs, ))
    out = net(inputs)
    netsize=count_param(net)
    print(out.size(),"params:%0.3fM"%(params/1000000),"(%s)"%netsize)
    print("FLOPS:%0.3fG"%(flops/1000000000),"(%s)"%flops)
    input("按任意键结束")